...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="gaze estimation using binned loss function.")
    parser.add_argument(
        "--evalpath",
        dest="evalpath",
        help="path for evaluating gaze test.",
        default="evaluation\\L2CS-gaze360-_standard-10",
        type=str,
    )
    parser.add_argument(
        "--respath",
...
if __name__ == "__main__":
    ...
    with open(os.path.join(respath, "avg.log"), "w") as outfile:
        ...
        outfile.write(
            "min angular error equal= " + str(minimum) + "at epoch= " + str(epoch_counter) + "\n"
        )
    print(minimum)
