...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="datasets/Gaze360/Label/train.label",
        type=str,
    )
    # mpiigaze
    parser.add_argument(
    ...
    parser.add_argument(
        "--snapshot", dest="snapshot", help="Path of model snapshot.", default="", type=str
    )
    parser.add_argument(
        "--gpu",
        dest="gpu_id",
...
def getArch_weights(arch, bins):
    if arch == "ResNet18":
        ...
        if arch != "ResNet50":
            print(
                "Invalid value for architecture is passed! "
...
if __name__ == "__main__":
    ...
    if data_set == "gaze360":
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 1 == 0 and epoch < num_epochs:
                print(
                    "Taking snapshot...",
                    torch.save(
                        model.state_dict(), output + "/" + "_epoch_" + str(epoch + 1) + ".pkl"
                    ),
                )
        ...
        for fold in range(15):
            ...
            print("Loading data.")
            dataset = Mpiigaze(
                testlabelpathombined, args.gazeMpiimage_dir, transformations, True, fold
            )
            train_loader_gaze = DataLoader(
                dataset=dataset,
                batch_size=int(batch_size),
            ...
            if not os.path.exists(
                os.path.join(output + "/{}".format(summary_name), "fold" + str(fold))
            ):
