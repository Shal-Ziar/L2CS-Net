...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="datasets/Gaze360/Image",
        type=str,
    )
    parser.add_argument(
        "--gaze360label_dir",
        ...
        default="datasets/Gaze360/Label/test.label",
        type=str,
    )
    # mpiigaze
    parser.add_argument(
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    parser.add_argument(
        "--dataset", dest="dataset", help="gaze360, mpiigaze", default="gaze360", type=str
    )
    parser.add_argument(
        "--snapshot",
        dest="snapshot",
        help="Path to the folder contains models.",
        default="output/snapshots/L2CS-gaze360-_loader-180-4-lr",
        type=str,
    )
    parser.add_argument(
        "--evalpath",
        dest="evalpath",
        help="path for the output evaluating gaze test.",
        default="evaluation/L2CS-gaze360-_loader-180-4-lr",
        type=str,
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if data_set == "gaze360":
        ...
        gaze_dataset = Gaze360(
            args.gaze360label_dir, args.gaze360image_dir, transformations, 180, 4, train=False
        )
        ...
        for fold in range(15):
            ...
            gaze_dataset = Mpiigaze(
                testlabelpathombined, args.gazeMpiimage_dir, transformations, False, angle, fold
            )
            ...
            softmax = nn.Softmax(dim=1)
            with open(
                os.path.join(evalpath, os.path.join("fold" + str(fold), data_set + ".log")), "w"
            ) as outfile:
                ...
                for epochs in folder:
                    ...
                    saved_state_dict = torch.load(
                        os.path.join(snapshot_path + "/fold" + str(fold), epochs)
                    )
                    ...
                    avg_MAE.append(avg_error / total)
                    loger = (
                        f"[{epochs}---{args.dataset}] Total Num:{total},MAE:{avg_error / total} \n"
                    )
                    outfile.write(loger)
                    print(loger)
            ...
            plt.plot(epoch_list, avg_MAE, color="k", label="mae")
            fig.savefig(
                os.path.join(evalpath, os.path.join("fold" + str(fold), data_set + ".png")),
                format="png",
            )
            # plt.show()
